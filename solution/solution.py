...
def _provider_options() -> dict:
    """Tùy chọn OpenRouter, không gửi extension này đến provider khác."""
    effort = os.getenv("LAB_REASONING_EFFORT")
    if effort and os.getenv("OPENAI_BASE_URL", "").rstrip("/") == "https://openrouter.ai/api/v1":
...
def compare_models(prompt: str) -> dict:
    ...
    """
    main_text, main_latency = call_openai(prompt)
    mini_text, mini_latency = call_openai_mini(prompt)
    # Giá tham chiếu học tập theo đề, không phải hóa đơn endpoint miễn phí.
    cost = len(main_text.split()) / 0.75 / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
        "gpt4o_response": main_text,
    ...
    try:
        while True:
            ...
            stream = client.chat.completions.create(
                model=OPENAI_MODEL, messages=messages, stream=True, max_tokens=512,
                **_provider_options(),
            )
...
def _print_stream(stream) -> str:
    """In nội dung; bỏ chunk metadata rỗng và đóng stream kể cả khi lỗi."""
    pieces = []
    try:
        for chunk in stream:
...
def retry_with_backoff(
    ...
    if max_retries < 0 or base_delay < 0:
        raise ValueError("max_retries và base_delay phải không âm")
...
def run_assistant(
    ...
    if max_turns is not None and max_turns < 0:
        raise ValueError("max_turns phải không âm")
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    ...
    try:
        while max_turns is None or num_turns < max_turns:
            ...
            # Không phát lại stream đã in một phần nếu mạng lỗi giữa chừng.
            reply = _print_stream(stream)
            history.extend([{"role": "user", "content": user_msg},
                            {"role": "assistant", "content": reply}])
            history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    rows = [["Prompt", "Main response", "Mini response", "Main latency", "Mini latency"]]
    for result in results:
        rows.append([shorten(result["prompt"]), shorten(result["gpt4o_response"]),
                     shorten(result["mini_response"]),
                     f'{result["gpt4o_latency"]:.3f}s', f'{result["mini_latency"]:.3f}s'])
...
if __name__ == "__main__":
    print(f"Main: {OPENAI_MODEL}\nMini: {OPENAI_MINI_MODEL}")
    print("Chi phí hiển thị là ước tính tham chiếu học tập; không phải hóa đơn API.")
    print("=== So sánh model ===")
